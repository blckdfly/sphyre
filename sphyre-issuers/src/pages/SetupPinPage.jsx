...
export default function SetupPinPage() {
  ...
  useEffect(() => {
    ...
    const storedData = sessionStorage.getItem('temp_issuer_data');
    ...
    setIssuerData(JSON.parse(storedData));
  }, [navigate]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await issuerAuthService.setupAuth(issuerData, seedPhrase, pin);

      localStorage.setItem('issuerDID', issuerData.id);
      localStorage.setItem('issuerInfo', JSON.stringify(issuerData));

      sessionStorage.removeItem('temp_seed_phrase');
      sessionStorage.removeItem('temp_issuer_data');

      navigate('/');
    } catch (error) {
      console.error('Setup error:', error);
      setError(error.message || 'Failed to setup PIN. Please try again.');
      setLoading(false);
    }
  };
        ...
        <div style={styles.wrapper}>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.inputGroup}>
              ...
              <div style={styles.hint}>Exactly 6 digits</div>
            </div>
            ...
            <button
              ...
              style={{
                ...styles.button,
                ...(loading || pin.length !== 6 || confirmPin.length !== 6 ? styles.buttonDisabled : {})
              }}
              onMouseEnter={(e) => { if (!loading && pin.length === 6 && confirmPin.length === 6) e.currentTarget.style.transform = 'translateY(-2px)'; }}
