...
import { ChevronLeft, Database, Trash2, RefreshCw, CheckCircle, AlertCircle, Clock } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-light-100">
      ...
      <div className="bg-white px-4 py-4 flex items-center border-b border-dark-100">
        ...
        <h1 className="text-lg font-semibold text-dark-500">Cached Contexts</h1>
      </div>
      ...
      <div className="px-4 py-6">
        <div className="bg-white rounded-xl p-5 shadow-sm mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              ...
              <h2 className="text-lg font-semibold text-dark-500">Storage Summary</h2>
            </div>
            <button 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-light-200 rounded-lg p-3">
              <p className="text-sm text-dark-300 mb-1">Total Contexts</p>
              <p className="text-xl font-semibold text-dark-500">{contexts.length}</p>
            </div>
            <div className="bg-light-200 rounded-lg p-3">
              <p className="text-sm text-dark-300 mb-1">Total Size</p>
              <p className="text-xl font-semibold text-dark-500">{totalSize}</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-dark-300 uppercase tracking-wider px-1">
            Cached Contexts
          </h3>
          ...
          {contexts.length > 0 ? (
            contexts.map((context) => (
              <div
                key={context.id}
                className="bg-white rounded-xl p-4 shadow-sm border border-dark-100"
              >
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center">
                      <h4 className="text-sm font-medium text-dark-500">{context.name}</h4>
                      <div className="ml-2">
                        {getStatusBadge(context.status)}
                      </div>
                    </div>
                    <p className="text-xs text-dark-300 mt-1 break-all">{context.url}</p>
                    <div className="flex items-center mt-2">
                      <p className="text-xs text-dark-300 mr-3">Size: {context.size}</p>
                      <p className="text-xs text-dark-300">Updated: {formatDate(context.lastUpdated)}</p>
                    </div>
                  </div>
                  <div className="flex space-x-2">
                    <button 
                      onClick={() => handleRefreshContext(context.id)}
                      className="p-1.5 rounded-full text-primary-500 hover:bg-blue-100"
                    >
                      <RefreshCw size={16} />
                    </button>
                    <button 
            ...
            <div className="flex flex-col items-center justify-center py-12 bg-white rounded-xl shadow-sm">
              ...
              <p className="text-dark-300 text-center">No cached contexts</p>
              <p className="text-dark-200 text-sm text-center mt-1">
