...
pub struct PresentationRequestRequest {
    ...
    pub required_credentials: Vec<crate::models::CredentialRequirement>,
    pub purpose: String,
...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/credential-offer",
            post(generate_credential_offer_short_url),
        )
        .route(
            "/presentation-request",
            post(generate_presentation_request_short_url),
        )
}
...
async fn generate_credential_offer_short_url(
    ...
    let qr_service = state.qr_service();
    let short_id = qr_service
        .generate_credential_offer_short_url(
            &request.issuer_did,
...
async fn generate_presentation_request_short_url(
    ...
    let short_id = verifier_service
        .generate_presentation_request_qr_short_url(
            ...
            None, // No predicates for this QR service
        )
        .await?;
