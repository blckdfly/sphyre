...
use crate::utils::did::{pq_sign, pq_verify, sign};
...
pub fn create_jwt(
    ...
    let signature = match sign(signature_input.as_bytes(), private_key_base58) {
        Ok(sig) => sig,
        Err(_) => {
            let mut hasher = Sha256::new();
            hasher.update(signature_input.as_bytes());
            hasher.update(private_key_base58.as_bytes());
...
pub fn extract_credential(jwt: &str) -> Result<Value, AppError> {
    ...
    claims.additional_claims.get("vc").cloned().ok_or_else(|| {
        AppError::SsiError("JWT does not contain a verifiable credential".to_string())
    })
}
...
pub fn extract_presentation(jwt: &str) -> Result<Value, AppError> {
    ...
    claims.additional_claims.get("vp").cloned().ok_or_else(|| {
        AppError::SsiError("JWT does not contain a verifiable presentation".to_string())
    })
}
...
pub fn create_pq_jwt(
    ...
    let signature = match pq_sign(signature_input.as_bytes(), private_key) {
        Ok(sig) => sig,
        Err(_) => {
            let mut hasher = Sha256::new();
            hasher.update(signature_input.as_bytes());
...
pub fn create_pq_credential_jwt(
    ...
    claims
        .additional_claims
        .insert("vc".to_string(), credential);
    claims
        .additional_claims
        .insert("pqk".to_string(), json!(hex::encode(public_key)));
...
pub fn create_pq_presentation_jwt(
    ...
    claims
        .additional_claims
        .insert("vp".to_string(), presentation);
    claims
        .additional_claims
        .insert("pqk".to_string(), json!(hex::encode(public_key)));
...
pub fn verify_pq_jwt(jwt: &str) -> Result<(JwtHeader, JwtClaims), AppError> {
    ...
    if header.alg != "Dilithium" {
        return Err(AppError::SsiError(
            "JWT is not signed with Dilithium".to_string(),
        ));
    }
    ...
    let public_key_hex = claims
        .additional_claims
        .get("pqk")
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
            AppError::SsiError("JWT does not contain a post-quantum public key".to_string())
        })?;
