...
use crystals_dilithium::dilithium2::{
    PublicKey, SecretKey, Signature, PUBLICKEYBYTES, SECRETKEYBYTES, SIGNBYTES,
};
...
) -> io::Result<([u8; KYBER_PUBLICKEYBYTES], [u8; KYBER_SECRETKEYBYTES])> {
    let mut rng = OsRng;
    let keypair_result = keypair(&mut rng).map_err(|e| {
        io::Error::new(
            io::ErrorKind::Other,
...
) -> io::Result<([u8; KYBER_CIPHERTEXTBYTES], [u8; KYBER_SYMBYTES])> {
    let mut rng = OsRng;
    let result = encapsulate(public_key, &mut rng).map_err(|e| {
        io::Error::new(
            io::ErrorKind::Other,
...
) -> io::Result<[u8; KYBER_SYMBYTES]> {
    let result = decapsulate(ciphertext, secret_key).map_err(|e| {
        io::Error::new(
            io::ErrorKind::Other,
            format!("Kyber decapsulation failed: {}", e),
        )
    })?;
...
pub fn decrypt_with_kyber(
    ...
    let kyber_ciphertext = encrypted_data[..KYBER_CIPHERTEXTBYTES]
        .try_into()
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "Invalid Kyber ciphertext"))?;
...
/// Sign a message using Dilithium Returns the signature or an error
pub fn dilithium_sign(message: &[u8], secret_key_bytes: &[u8]) -> io::Result<Vec<u8>> {
    // Validate secret key size
    if secret_key_bytes.len() != SECRETKEYBYTES {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
                "Invalid Dilithium secret key size: expected {}, got {}",
                SECRETKEYBYTES,
