...
impl AuthService {
    ...
    pub async fn login(&self, request: LoginRequest) -> Result<AuthResponse, AppError> {
        ...
        let user = self
            .db
            .find_user_by_did(&request.did)
            .await?
            .ok_or_else(|| {
                AppError::AuthError(format!("User with DID {} not found", request.did))
            })?;
    ...
    pub fn generate_token(&self, user: &User) -> Result<String, AppError> {
        ...
        claims.additional_claims.insert(
            "name".to_string(),
            serde_json::to_value(user.name.clone()).unwrap(),
        );
        claims.additional_claims.insert(
            "email".to_string(),
            serde_json::to_value(user.email.clone()).unwrap(),
        );

        let private_key = "MHcCAQEEIBzFxKC2KGaw+hQLKe8GTFFGVPno0FJ2vnggh5bN0qcgoAoGCCqGSM49AwEHoUQDQgAErltVqmiDOw4C3vbWKRPunqEyoVGj5trhSopY+IPW7ksD4zeVse7CB2elgcQorS/YYcF1AL7jLX6o6p5TWHeETA";
    ...
    pub async fn get_user_from_token(&self, token: &str) -> Result<User, AppError> {
        ...
        let did = claims
            .sub
            .ok_or_else(|| AppError::AuthError("Token missing subject".to_string()))?;
    ...
    pub async fn create_did_document(
        &self,
        request: GenerateDIDRequest,
    ) -> Result<DidKeyPair, AppError> {
